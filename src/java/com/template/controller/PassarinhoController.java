...
public class PassarinhoController {
    ...
    private final PassarinhoService passarinhoService;


    // Service recebido por injeção
    public PassarinhoController(
            PassarinhoService passarinhoService
    ) {
    ...
    private void btnCadastrarAction() {
        ...
        try {
            ...
            passarinhoService.cadastrar(
                    txtEspecie.getText(),
                    chkCativeiro.isSelected(),
                    spnIdade.getValue()
            );

            finalizarAcao(
                    "Passarinho cadastrado com sucesso!"
            );
    ...
    private void btnExcluirAction() {
        ...
        boolean confirmou =
                DialogUtil.mostrarConfirmacao(
                        "Confirmar",
                        "Deseja excluir '"
                                + selecionado.getEspecie()
                                + "'?"
                );
        ...
        try {
            ...
            passarinhoService.deletar(
                    selecionado.getId()
            );

            finalizarAcao(
                    "Passarinho removido com sucesso!"
            );
    ...
    private void carregarPassarinhos() {
        ...
        try {
            ...
            tblPassarinho.setItems(
                    FXCollections.observableArrayList(
                            passarinhoService.listarTodos()
                    )
            );
    ...
    private void finalizarAcao(String mensagem) {
        ...
        Servicos.mostrarMensagemSucesso(
                lblMensagem,
                mensagem
        );
    }
